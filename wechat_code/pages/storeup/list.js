...
// 页面显示时执行的操作
    ...
    async search() {
        ...
        params['sort'] = 'id';
        params['order'] = 'asc';
        params['type'] = wx.getStorageSync('menuJump')
        if (this.data.name) {
...
// 返回顶部时隐藏按钮
    ...
    async getData() {
        const obj={
            page: 1,
            limit: 100,
            type:wx.getStorageSync('menuJump')
        }
        ...
        if (userId) {
            obj['userid']=userId;
        const responseList = await  page("storeup", obj);
            data = responseList.data;
        } else {
            const responsePage = await list("storeup", obj);
