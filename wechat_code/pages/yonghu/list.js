...
if(this.data.zhanghao){
    searchForm['zhanghao'] = '%' + this.data.zhanghao + '%'
}
if(this.data.xingming){
    searchForm['xingming'] = '%' + this.data.xingming + '%'
}
...
wx.navigateTo({
url: `/pages/yonghu/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/yonghu/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 yonghu Yonghu yonghu ${pathname}
