...
res = await page(`systemnotice`, searchForm);
} else {
res = await list(`systemnotice`, searchForm);
}
...
wx.navigateTo({
url: `/pages/systemNotice/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("system_notice",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/systemNotice/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/systemNotice/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 system_notice SystemNotice systemNotice ${pathname}
console.log('SystemNotice  system_notice')
isAuthObj['addAuth']   = utils.isAuth("system_notice", "新增")
isAuthObj['delAuth']  = utils.isAuth("system_notice", "删除")
isAuthObj['editAuth']   =utils.isAuth("system_notice", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("system_notice", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("system_notice", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("system_notice", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("systemnotice",obj)
    }else{
         resList= await list("systemnotice",obj)
    }
