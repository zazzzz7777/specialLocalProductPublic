...
if (id) {
const {data} = await detail("systemnotice",id)
this.setData({
payAuth:utils.isAuthFront('system_notice','支付')
})

let objClo={}
data.content=utils.addImgWidthStyle(data.content,'100%')
...
const resUpdate = await update('systemnotice', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','system_notice');
...
async reply() {
const detailList = this.data.detailList
const res = await update("system_notice", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','system_notice');
